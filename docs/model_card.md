...
| 金属音检出率 | 0.0119（1/84） | **0.0357（3/84）** | 本模型略增，样本量小，需盲听复核 |
| 说话人相似度（对参考音频） | 0.9935 | 0.9942 | **无下降**，参考音频克隆能力未受损 |
| 语速（字/秒） | 10.78 | 9.41 | 见下条：不是变慢，是尾部静音变长 |
| 有声段语速（字/秒） | 11.52 | 11.32 | **−1.7%，基本没变** |
| 尾部静音 | 0.084 s | **0.292 s** | 最长 0.46s、p90 0.42s，**0/84 越过 0.5s 上限** |

`speaker_sim` 用的是 MFCC 余弦（OmniVoice 的 `mfcc_v1` 回退档，标注为低可信），
绝对值都挤在 0.99 附近、动态范围很小，**只能用来判断"有没有明显损坏"，不能分辨细微音色差异**。
要精确结论需换真实说话人嵌入（如 modelscope ERes2NetV2）或母语盲听。
