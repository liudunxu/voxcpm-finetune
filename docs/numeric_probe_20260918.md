...
修复提交：DIS `1a1e309`、OmniVoice `ac1e51d`，已随后续修复push，尚未部署。
...
后续独立版本的纯文本边界修复已完成，见下节；**它不回写此冻结评分器或本轮分数**，
不顺便猜测时钟规则或将不同币种视为等价。
...
2026-09-18修复DIS的`normalize_clone_numbers`与OmniVoice的`verbalize_locale_numbers`，
同时覆盖它们的合成文本与文本QC调用者，不另写评测专用纠错器。

- 四语种识别已有标记的并集`Rp/RM/฿/₱/$/€/£/¥/₫`及其后空白，
  **仅按各语种原有币种表展开**。MS的`Rp45,000`和`Rp 45,000`均原样保留，
  不改金额、不把Rp映射成RM；合法`RM 1,200 ringgit`仍只保留一个币种名称。
- 数字正则只能匹配连续数字串的一段时，拒绝展开该片段：
  `RM1,23,456`、`Rp1.23.456`与标识符后的数字片段不再被分段改写。
  普通句读后的`Harga,5.`与带单位列表`2kg,3kg`仍正常词化，
  没有用“凡是逗号/句点后都禁止”的宽泛补丁。
- 币种后带正负号（如`RM -45,000`）也保留，不新增未经确认的读法。
  范围限于上述标记与已有数字语法，**不是任意ISO币种、`Rp.`等缩写、日期或时钟解析器**。
  EN/TL规则、CFG/步数/seed策略与词化开关默认不变；false覆盖值仍不词化。
- 服务端结果缓存算法10→11；DIS文本签名TH 3→4，VI/ID/MS 2→3。
  实际旧/新缓存函数在相同受控输入下，服务端key及四语种签名均改变，TL/EN签名不变；
  这是函数执行检查，不是在线缓存验证，也没有删除旧音频。

验证与影响范围：

- 两侧各**21项回归**，直接执行旧代码各19失败/2通过，当前代码全过；
  另覆盖真实服务端normalizer的true/false开关、DIS实际payload与文本QC、幂等性。
  扩展DIS **198项**、OmniVoice **254项**通过；服务端仅有既有依赖弃用告警。
- 两侧对**1,296个外来币种标记/金额分隔符/空白/符号位置组合**均保持原文，
  这是含重合输入的代码组合检查，不是1,296条模型评测或母语样本。
- 重新执行原29条DIS输入，**只有`id_digit_3`的payload改变**：
  `pukul nol enam.lima belas`→`pukul 06.15`。只是停止局部展开，不声称正确念出时钟；
  其余28条保持不变，29条经过服务端词化函数均不再变化。此条不在已完成的125条生成中，
  本轮没有为它追加GPU或ASR任务。
- 原6份收尾指纹产物、冻结`locale_text.py`和125WAV再次通过SHA256核对；
  原共同CER、ASR转写与验收字段完全不改。

两仓已push：DIS **`9a961b2`**、OmniVoice **`88d9dd2`**，尚未部署。
DIS远端新增`25592cd`，已保留并合并为`be6e735`后正常推送，合并后69项相关测试通过；
未强推或改动既有未跟踪`docs/reports/`。归档`closure.json`的`pushed=false`是归档当时状态，
保留原文件，不为后续push改写冻结证据。
测试日志、旧代码反证、缓存函数验证、29条payload与源码指纹另存
`checkpoints/numeric_text_boundary_20260918/`，对应远端大盘同名归档；
两仓补丁仅作证据备份，不安装到微调或生产服务。

## 下一步与停止条件

1. 保留已确认的重复币种输入修复；不全局启停数字词化、不调CFG/步数、不加epoch、不晋级新模型。
2. 币种/分隔符纯文本回归已完成；不为得到更好分数重算旧报告，
   不扩TTS/ASR网格，暂无新证据支持启动下一轮训练。
3. 有可核验服务时独立核对部署commit、权重、最终文本/采样参数与缓存版本，
   再决定最小真实调用验证；缺服务不妨碍本轮离线工程修复收尾。
   金额发音、口音、自然度与情绪继续未验证，不要求用户作五语种母语判断。
4. 当前暂停训练GPU；有新的可复现坏例，或可追溯数据与明确增益假设时，
   先冻结回归和单变量方案再开机，不为等待线上核验空转，也不因缺drama或母语评分永久搁置微调。
