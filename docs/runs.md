...
**2026-09-23紧邻同人身份/音色复盘（非训练run）。**
两片`mtgg3m0`/`mtggf9f`已核DIS `7185f04`生效。首片用户确认首句偏男与1/2音色跳变；
后片1/2/3同人却仍沿用A–B–A身份，另有同人变声反馈。29份ref/prompt已校SHA，
交叉匹配恢复4份已采纳raw，未补造GPU处理后参考；回归只作开发诊断，不入训练。
DIS两处最小修复：高可信同声连续校验不受角色标签绕过；最终候选身份权重不因软风格告警降档。
新增3回归原代码全失败；修后三组144通过（4.37s）、162通过（4.90s）、37通过（0.89s），共343通过。
用户要求push后部署重识别：交付前16文件联跑343通过（8.09s）；DIS `b5f4fd4`已push到
`feat/recognition-generation-mainline`。仍未部署/重识别/重渲，未做新听评；生产仍r8，旧24条诊断0生成。
证据与未决项见`docs/video_review_20260923_adjacent_identity.md`；不以调用方修复宣称模型坏例消除。

**2026-09-23局部背景盲听反馈（非训练run）。**
用户提交`mix-local-20260923`：M01/M02均“差不多”，备注均为空；本轮2平，无胜负偏好。
按本对话最近交付入口关联`pack_v2`（用户JSON未单列版本），原文保存于
`work/audio_ab/20260923_mix_peak/pack_v2/feedback.json`；M01为A关/B开，M02为A开/B关。
冻结清单SHA256：`796ccb7d7fbf2ef01500966fa27c08282a51a9f66290c6f8815a9306422db8cf`。
18份既有产物SHA复核一致；未改清单、测量、页面或WAV，未重新生成或评分。
仅说明这两组TH样本未显示局部压背景的偏好收益，保持默认关闭，不外推策略普遍等效。
未给绝对声学正常评价，不收为正常候选；语言自然度仍未验证，也不是VI沙哑或读稿感改善证据。
后续仍按既有沙哑/读稿感方案定位实际take，不因本反馈增加GPU、ASR、TTS或训练预算。

**2026-09-23局部混音/编码峰值与读稿感策略（非训练run）。**
DIS `7185f04`补AAC实际解码峰值检查，超标从原PCM固定衰减重编码一次，视频包/字幕/发布数量不变。
局部400ms背景平衡仅供候选实验、默认关闭；另修复侧链开启且渐变0时全局对白背景衰减失效。
旧代码负向1失败21通过，修后11个相关文件132通过（20.94s）；新增测试使用真实FFmpeg检查音频与封装。
两个既有TH人声/背景固定输入的混音盲听包已冻结，18份产物SHA核验通过，无新TTS/ASR。
入口`work/audio_ab/20260923_mix_peak/pack_v2/index.html`，含原ref与请求白名单、原WAV字节及SHA；
正常片段各一次编码；另做电平压力测试，峰值−0.44/−0.19 → −1.72/−1.98dBTP，不是VI沙哑复现。
用户新反馈“像读稿子、AI味太重”已登记；核现有口语审核、话轮/停顿、控制/旁白标记与变速链路，
方案见`docs/natural_dialogue_strategy_20260923.md`。未指定的新片段不冒充已确认读稿case。
保持r8，旧24条模型计划仍0生成，未部署/未重渲/未训练；声音及五语种自然度改善尚未验证。

**2026-09-23整体质量复查与参考持久化（非训练run）。**
9片实际音轨为44.1/48kHz AAC；另测4片LUFS/真峰值，最新VI为-14.63 LUFS/-0.05dBTP。
据此先验证局部混音与峰值余量，不全局加音量/采样率。工程自愈与定向模型实验并行准备，
不是直接续训r8：原24条模型计划仍0生成、正式真人训练音频0，生产模型不变。
已补10条正常合成候选的ref/prompt侧录，实际ref去重2条；16条回归中9条参考恢复为7个去重WAV。
全部仅开发诊断，原试听/冻结数据不改；客户端ref不冒充GPU最终输入，未知项显式保留。
上一批三仓远端已核对：`40b8954`/DIS`f30291c`/OmniVoice`11302c7`；DIS保留远端新增改动后
20文件824回归通过（10.28s）。本轮研究路线见`docs/quality_roadmap_20260923.md`，不声称听感改善。
参考侧录工具的本轮全套离线回归154通过（12.75s），备料池字节/关联自检通过；无新GPU或ASR调用。

**2026-09-23正常A/B候选归档与跨层重试修复（不是训练run）。**
用户要求A/B时顺便收集好片段：已从三个有反馈的旧包核SHA复制10条/39.02s到
`work/for_train/ab_candidates`，8 TL/2 ID；1条已有声学正常+自动文本pass/无severe，9条仍待QC复核。
全部合成来源、语言未验、development-only、training_eligible=false，不计真人训练时长；原包不改。
emotion两侧异常不收录，避免把相对B更好洗成干净正例；新增导出器仅本地归档，不触发新生成。
DIS跨层重试以累计请求序号派seed，拒绝回滚不再重复同一探索，首take/软锁和预算不变。
没有新GPU调用、微调或模型替换；原12+新增12模型归因计划仍0生成，需独立GPU入口。
交付复核：微调仓全套153通过（107.70s）；DIS17个相关文件698通过（9.44s）；备料池自检通过。
上述耗时仅为本地测试，不代表生产p95或音质收益；代码尚未部署。

**2026-09-23新片追加与训练备料（仍非训练run）。**
`mtg6jbj`确认DIS `bd2ada7`包及严重风险额外预算生效；用户1:16/cue15、1:53/cue25仍沙哑。
两句共用4.14s客户端ref，cue25已无control，不能仅归因于情绪；相同参考风险不等于已证模型缺陷。
新增独立12条base/r8计划，旧12不改；**原12和新增12均0条生成**，没有新GPU/训练/模型替换。
`work/for_train`准备14个来源候选、16条回归/反馈、393条去重评测文本排除；已审真人训练音频0。
6个公开来源仅取元数据，不在本机下载语料、不把成片/测试波形混入训练。详见
`docs/training_material_pool_20260923.md`与`docs/video_review_20260923_vi_mtg6jbj.md`。
DIS本地长空隙补识别修复通过483项相关回归（7.19s），未部署/重识别；不是模型实验结果。
