...
**2026-09-23新增坏例归因准备（非训练run，12条计划、0条生成）。**
两条VI严重沙哑反馈已冻结；E01情绪开关A/B听评两者失败，不能以偏好B宣称修复。
计划用基座`32279effe8c19989596f05d353d1447f51d9e915`及已备份r8，固定每条输入，
各跑原有效seed/+1/+7、官方LoRA关闭/开启，共12条。r8三文件和两参考SHA已本机核验，
执行器复用`infer.synthesize_ab`，不改训练算法、生产参数、旧报告或模型权重。
当前现有实例仅有生产API入口，尚无可用独立GPU执行通道，未新租/停机/部署/开始合成。
范围、模型指纹及命令见`docs/video_review_20260923_vi_mtg3tot.md`；不是r12，也不是已取得模型结论。
交付复核：微调仓全套152项通过（26.61s），DIS相关26文件367项通过（9.46s）；
DIS `bd2ada7`已push，用户部署待确认；OmniVoice无待交付修改，保留`11302c7`。
这次提交不是训练run，不改历史训练数字、权重或冻结音频。
...
关机前已备份r8及r10/r11 A/B共5份latest LoRA、配置与步数状态至本机
`checkpoints/gpu_pause_20260918/weights/`，15文件SHA全部通过，r8与数字实验冻结指纹一致。
校验记录为同目录上层`verification.json`；未完整备份训练数据、历史评测音频或优化器，
可以关机，但不能把这份关键产物备份当成释放实例/删除数据盘的依据。权重不入Git。
